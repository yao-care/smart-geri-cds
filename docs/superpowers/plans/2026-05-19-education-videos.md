...
**Goal:** 為 CDSA 長者發展評估 + CDSS 生理警示兩條結果線，建立 123 個 trigger → YouTube 衛教影片的多對多映射，含自動 curate 腳本（yt-dlp + heuristics + Claude Code 複審）與前端整合。
...
Run:
```bash
npx @lhci/cli collect --url=http://localhost:4321/smart-geri-cds/result/ --numberOfRuns=3
```
...
```yaml
# Tier 1: 台灣官方醫療頻道（國健署、衛福部、各醫學中心老年醫學部門、學會）
...
describe('simplifiedRatio', () => {
  it('returns 0 for pure traditional', () => {
    expect(simplifiedRatio('長者體重發展')).toBeLessThan(0.1);
  });
...
Run: `pnpm dev`
Expected: 開 `http://localhost:4321/smart-geri-cds/result/`，完成假評估後底部顯示影片區塊。
...
```json
{
  "ci": {
    "collect": {
      "url": [
        "http://localhost:4321/smart-geri-cds/result/",
        "http://localhost:4321/smart-geri-cds/workspace/result/"
      ],
      "numberOfRuns": 3,
