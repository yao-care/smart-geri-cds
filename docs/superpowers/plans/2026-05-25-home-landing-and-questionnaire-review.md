...
- **誠實底線（不可違反）**：信任文案只能寫「架構與國健署學前兒童發展檢核表／ASQ-3 一致」「參考通用發展里程碑」「經兒科專業人員審閱（2026-05）」。**禁止**寫「採用標準化量表」「經信效度驗證」「具名背書」。
...
```json
{
  ...
  "clinicalReview": {
    ...
    "reviewer": "未公開揭露",
    "basis": "題目參考通用兒童發展里程碑編寫（部分啟發自 Denver II / ASQ-3 / M-CHAT，見各題 source 欄位）；六大發展領域架構與國民健康署『學前兒童發展檢核表』及 ASQ-3 一致。本工具為發展分流參考，未經信效度驗證。"
  },
  "questions": [
```
...
- 44 題 clinicallyReviewed false→true，移除家長端「未審」徽章
...
第一個測試的標題與斷言保留（`/assess` 上仍渲染 shell 與「兒童基本資料」heading）。
...
const trust = [
  ...
  { title: '參考通用里程碑', desc: '六大發展面向架構與國民健康署「學前兒童發展檢核表」、ASQ-3 一致；題目參考通用發展里程碑編寫，並經兒科專業人員審閱（2026-05）。' },
];
---
...
- 信任段守誠實底線：架構一致、參考里程碑、經專業審閱，不寫採用量表/驗證
...
const features = [
  { title: '家長自評 (CDSA)', description: '家長可透過引導式問卷評估孩子健康狀況，取得即時衛教建議' },
  { title: '臨床決策輔助 (CDSS)', description: '醫護人員可即時監測兒童健康指標，接收智慧預警與風險分級' },
  { title: '智慧預警引擎', description: '即時偵測兒童健康指標異常，自動分級預警' },
...
```astro
  <section class="about-section about-twocol" aria-label="系統介紹">
    <div class="twocol-main">
      <h2>系統介紹</h2>
      <p>
        Smart Pedi 是一套開源的兒科健康輔助系統，提供雙角色服務：
        家長可透過 CDSA（臨床決策輔助應用）進行孩子健康自評並取得衛教建議；
