...
### 目標
為「引流」服務：讓**冷／半冷流量的照顧者**（主要來自 Google 搜尋與照顧者社群／口碑分享，非醫院轉介）一進首頁就能在數秒內理解「這是什麼、安不安全、要多久、能得到什麼」，並順暢進入評估。

### 受眾（已確認）
對孩子發展有疑慮的照顧者，主力管道 = **SEO + 社群分享**（暖流量的醫師轉介為次要，但設計需相容）。
...
### 不在本次（明確排除）
- 生命徵象規則／基線（`geriatric-default.yaml`、`geriatric-baselines.json`）的來源補登：**未經本次審閱確認**，維持原狀，不在範圍。
...
1. **Hero**：H1 價值主張（含照顧者會搜的關鍵詞）＋ 副標（給誰、解決什麼）＋ 主 CTA「開始評估」（→ `/assess`）＋ 微信任條（免費・免登入・隱私・約 N 分鐘）。
...
1. **系統介紹** 右側 → **重點摘要卡**（at-a-glance）：雙角色（照顧者／醫護）、隱私優先、開源免費、SMART on FHIR。
...
| 「六大發展面向與國健署『學前長者發展檢核表』、ASQ-3 等通用篩檢工具**架構一致**」 | 「**採用**國健署檢核表／ASQ-3 標準化量表」 |
| 「題目**參考**一般長者發展評估指標編寫」 | 「**經信效度驗證**」 |
| 「題目經**老年醫學專業人員審閱**（2026-05）」（用戶提供之事實） | 具名背書（用戶要求先不揭露） |
| 明確免責：非診斷、僅供參考、請諮詢專業 | — |

**量表比對依據**（2026-05-25 查證）：44 題的 6 大網域（粗動作／精細動作／語言理解／語言表達／認知／社會情緒）在領域架構上與國健署學前長者發展檢核表、ASQ-3（5 領域）高度一致；題目皆為跨工具共通的通用評估指標。但題目為人工自編（`source: manual`），年齡分段、題幹、0/1/2 計分為自訂，未對應任何單一標準化量表的題目與常模，亦未做信效度驗證。故只能宣稱「架構一致／參考評估指標」，不能宣稱「採用標準化量表／經驗證」。
...
4. `source`：**維持 `manual`**（題目確為自編，誠實）；另加 `basis` 欄位記錄透明度資訊，例：`"basis": "通用發展評估指標；領域架構對齊國健署學前長者發展檢核表 / ASQ-3"`。
...
- `/` 的 `<title>` 與 `<meta description>` 含照顧者搜尋關鍵詞（衰弱失能、語言遲緩、發展篩檢、長者發展評估）。
- H1 含關鍵詞、語意清楚。
